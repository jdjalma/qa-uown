...
test.describe(`Invoice Cancellation at APPROVED`, { tag: splitTags(approvedCancellationData.tag) }, () => {
  test('Cancel invoice items at APPROVED and verify INVOICE_CANCELED status', async ({ page, api, ctx, merchantConfig: mSetup }) => {
    ...
    const { env, merchant, applicant } = buildTestData({
      state: approvedCancellationData.state,
      merchant: approvedCancellationData.merchant,
      orderTotal: '800',
      orderDescription: 'Invoice cancellation test',
    });

    await test.step('Ensure merchant config', async () => {
      await mSetup.configureByName(approvedCancellationData.merchant, 'lifecycle');
    });
...
test.describe(`Lease Cancellation at SIGNED`, { tag: splitTags(signedCancellationData.tag) }, () => {
  test('Cancel lease at SIGNED and verify UW_APPROVED status', async ({ page, api, ctx, merchantConfig: mSetup }) => {
    test.setTimeout(300_000);
    const { env, merchant, applicant } = buildTestData({
      state: signedCancellationData.state,
      merchant: signedCancellationData.merchant,
      orderTotal: '800',
      orderDescription: 'Signed cancellation test',
    });

    await test.step('Ensure merchant config', async () => {
      await mSetup.configureByName(signedCancellationData.merchant, 'lifecycle');
    });
...
test.describe(`Lease Cancellation at FUNDING (no refund)`, { tag: splitTags(fundingNoRefundData.tag) }, () => {
  test('Cancel lease at FUNDING after payments in servicing + customer portal, no refund', async ({ page, api, email, ctx, merchantConfig: mSetup }) => {
    ...
    const { env, merchant, applicant, address } = buildTestData({
      state: fundingNoRefundData.state,
      merchant: fundingNoRefundData.merchant,
      orderTotal: '800',
      orderDescription: 'Funding cancellation no refund test',
    });

    await test.step('Ensure merchant config', async () => {
      await mSetup.configureByName(fundingNoRefundData.merchant, 'lifecycle');
    });
    ...
    await test.step('Make CC payment in servicing portal', async () => {
      ...
      const billing = { address: address.street, city: address.city, state: fundingNoRefundData.state, zip: address.zipCode };
      await svcCustomerPage.makeCcPayment('0', fundingNoRefundData.servicingPaymentAmount, buildCcPaymentDetails(card, billing));
      console.log(`[Test] CC payment of $${fundingNoRefundData.servicingPaymentAmount} made in servicing portal`);
    });

    await test.step('Make CC payment in customer portal (website)', async () => {
      await makePaymentOnCustomerPortal(page, env, email, applicant.email, fundingNoRefundData.customerPaymentAmount);
    });
...
test.describe(`Lease Cancellation at FUNDING (with refund)`, { tag: splitTags(fundingWithRefundData.tag) }, () => {
  test('Cancel lease at FUNDING after payments in servicing + customer portal, with refund', async ({ page, api, email, ctx, merchantConfig: mSetup }) => {
    test.setTimeout(600_000);
    const { env, merchant, applicant, address } = buildTestData({
      state: fundingWithRefundData.state,
      merchant: fundingWithRefundData.merchant,
      orderTotal: '800',
      orderDescription: 'Funding cancellation with refund test',
    });

    await test.step('Ensure merchant config', async () => {
      await mSetup.configureByName(fundingWithRefundData.merchant, 'lifecycle');
    });
    ...
    await test.step('Make CC payment in servicing portal', async () => {
      ...
      const billing = { address: address.street, city: address.city, state: fundingWithRefundData.state, zip: address.zipCode };
      await svcCustomerPage.makeCcPayment('0', fundingWithRefundData.servicingPaymentAmount, buildCcPaymentDetails(card, billing));
      console.log(`[Test] CC payment of $${fundingWithRefundData.servicingPaymentAmount} made in servicing portal`);
    });

    await test.step('Make CC payment in customer portal (website)', async () => {
      await makePaymentOnCustomerPortal(page, env, email, applicant.email, fundingWithRefundData.customerPaymentAmount);
    });
    ...
    await test.step('Verify refunded payments in CC history', async () => {
      ...
      expect(
        refundedAmount?.trim(),
        `Refunded amount should match servicing payment of $${fundingWithRefundData.servicingPaymentAmount}.00`,
      ).toContain(`$${fundingWithRefundData.servicingPaymentAmount}.00`);
    });
  });
});
