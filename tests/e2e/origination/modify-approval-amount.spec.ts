...
test.describe(`Modify Approval Amount - ${testData.state}/${testData.merchant}`, { tag: testData.tag.split(' ') }, () => {
  test('Creating Uown account', async ({ page, api, ctx, merchantConfig: mSetup }) => {
    ...
    await test.step('Ensure merchant config', async () => {
      await mSetup.configureByName(testData.merchant, { ...({ maxApprovalAmount: MAX_APPROVAL_AMOUNT, fraudThreshold: 900 }) });
    });
    ...
    await test.step('Login as merchant and verify button is NOT visible', async () => {
      ...
      if ((process.env.ENV || 'sandbox') !== 'sandbox') {
