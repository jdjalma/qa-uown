/**
 * RU06.26.1.53.0 — Complete Application security: PII data access reduced
 *
 ...
 *   tests/e2e/origination/RU06.26.1.53.0_completeApplicationSecurityPiiDataAccessReduced.spec.ts
 */
 ...
 */
// -- ORACLE SCOPE (calibrated 2026-06-19, CT-02 debug; F-003 finalized) ---
// The fix removed @persist from EXACTLY basicCustomerData + protectionPlan in
...
// page to completeApplication") -- i.e. PRE-fix. On submit the FE persists the
...
// .seonInfo.birthDate / .fullName, @persist desde 2021) — FORA do escopo da demanda,
// finding SEPARADO (ver report F-003; decisão do user 2026-06-19: tratar como
// finding à parte, NÃO como bug da demanda). DB-/DOM-proven em qa2 nos fresh leads
...
// submitApplicationResponse. The probes the fix MUST keep strict —
...
async function assertNoPiiAtRest(page: Page, setup: SetupResult, label: string): Promise<void> {
  ...
  // (@persist since 2021) is OUT of the demand scope ([OBSERVAÇÃO], F-003) and stripped;
...
async function setupApplication(opts: {
  ...
  // false → Buddy never renders (F-005). The fallback lives in
...
test.describe(
  'RU06.26.1.53.0_completeApplicationSecurityPiiDataAccessReduced',
  ...
  () => {
    ...
    test('CT-01: basicCustomerData absent from localStorage AND from the gate response (non-SEON/non-insurance)', async ({
      ...
      try {
        ...
        await test.step('CT-01: gate response carries NO basicCustomerData (negative gate) [#8]', async () => {
          ...
          if (gate === GATE_NOT_CAPTURED) {
            ...
            // ORACLE SCOPE (calibrated 2026-06-19, CT-01 debug): the negative
            ...
            // mis-fires. The mechanism is: no PII FIELD is populated. Assert that
      ...
      //
      // STATE-COLLISION FIX (calibrated 2026-06-19, CT-01 cycle-2 debug):
    ...
    test('CT-02: SEON gate response carries basicCustomerData (idCheckProvider==SEON)', async ({
      ...
      await test.step('CT-02: SEON evidence persisted, submission logged, no missing-data note [reflex]', async () => {
        // ORACLE SCOPE (calibrated 2026-06-19, CT-02 debug): SEON identity
