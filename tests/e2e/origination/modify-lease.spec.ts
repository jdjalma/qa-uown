...
for (const data of reduceValueData) {
  test.describe(
    `Modify Lease - Reduce Value (SIGNED) - ${data.env}`,
...
for (const data of increaseValueData) {
  test.describe(
    `Modify Lease - Increase Value (SIGNED) - ${data.env}`,
    { tag: splitTags(data.tag) },
    () => {
      test.use({ envName: data.env });
...
for (const data of apiModifyData) {
  test.describe(
    `Modify Lease via API - ${data.env}`,
    { tag: splitTags(data.tag) },
    () => {
      test.use({ envName: data.env });
...
for (const data of minimumAmountData) {
  test.describe(
    `Modify Lease - Minimum Amount Validation - ${data.env}`,
    { tag: splitTags(data.tag) },
    () => {
      test.use({ envName: data.env });
...
for (const data of fundingModifyData) {
  test.describe(
    `Modify Lease at FUNDING - ${data.env}`,
    { tag: splitTags(data.tag) },
    () => {
      test.use({ envName: data.env });
