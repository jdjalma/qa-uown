...
test.describe(`New Application - ${testData.state}/${testData.merchant}`, { tag: testData.tag.split(' ') }, () => {
  test('Creating Uown account via UI', async ({ page, api, email, ctx, merchantConfig: mSetup }) => {
    ...
    await test.step('Create a new application via merchant form', async () => {
      ...
      const merchantNorm = testData.merchant.replace(/[\s'&]/g, '').toLowerCase();
      ...
      if (!merchantSelected) {
        console.log(`[NewApp] WARNING: Could not find merchant "${testData.merchant}" in any dropdown`);
      }
