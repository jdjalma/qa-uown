...
test.describe(`Protection Plan Cancellation - ${testData.merchant}`, { tag: splitTags(testData.tag) }, () => {
  test('Full protection plan cancellation flow with sweep', async ({ page, api, ctx, merchantConfig: mConfig }) => {
    ...
    await test.step('Enable Offer Insurance on merchant via API', async () => {
      await mConfig.configureByName(testData.merchant, 'withInsurance');
      console.log(`[Phase 1] Offer Insurance enabled via API for ${testData.merchant}`);
    });
    ...
    await test.step('Make CC payment in servicing', async () => {
      ...
      await svcBase.makeCcPayment('0', testData.paymentAmount, buildCcPaymentDetails(card, billing));
    });
