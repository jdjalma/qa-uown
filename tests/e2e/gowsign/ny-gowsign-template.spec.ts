/**
 * E2E qa2 — New York GowSign Template
 *
 ...
 *
 * Routing recipe (see docs/knowledge-base/new-york-gowsign-template.md):
...
// forum boilerplate ("Tampa, FL"), expected in every contract (BR-01).
 ...
 * `NY_2025_SAC` contract during discovery (see
 * docs/knowledge-base/new-york-gowsign-template.md "Content Parity").
...
async function openNyGowSignContract(
  ...
  const { merchant, applicant } = buildTestData({
    ...
    orderDescription: 'NY GowSign template',
  ...
  // cardholder last name MUST equal the applicant last name (BR-02).
    ...
    test(
      ...
      async ({ page, api, ctx, db }, testInfo) => {
        ...
        await test.step('AC-03/AC-04: every key calculated value from the backend renders', async () => {
          ...
          // both providers (Content Parity). Compared against the exact
