...
/**
 * Page object for the Origination New Application page filter panel.
 *
 * The New Application page has a FilterTable with Merchant (single-select)
 * and Location (single-select, dependent on Merchant) filters.
 *
 * Endpoint: POST /uown/getSendApplicationRequestsByCriteria
 */
export class NewApplicationFiltersPage extends OriginationBasePage {

  async navigateToNewApplication(originationUrl: string): Promise<void> {
    await this.page.goto(`${originationUrl}/newApplication`, { waitUntil: 'domcontentloaded' });
