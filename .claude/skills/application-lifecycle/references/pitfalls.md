...
| 146 | **`#from`/`#to` on `/leads` (Origination) ignore `fill()`/`browser_type` — Formik does not update, Search does not fire `getLeadsByCriteria`.** Fix: React-fiber `onChange` via `page.evaluate` (same pattern as `setFormikDate` in `overview.page.ts`). | [03-pitfalls-138-141.md](pitfalls/03-pitfalls-138-141.md) |
...
| 146 | **A test asserting `reverse_date`/`status=REVERSED` immediately after an ACH Fully Refund fails, while the equivalent CC refund passes instantly.** CC Fully Refund is synchronous (completes within the same request); ACH Fully Refund is asynchronous (creates a new `ACHPayment` `status=PENDING, achProcessType=REFUND`; original `reverse_date` stays `NULL` until sweep/Profituity settlement). Pre-existing behavior, first documented via the Sticky reverse/refund regression (servicing#519, CT-06, sandbox 2026-06-30). | [03-pitfalls-138-141.md](pitfalls/03-pitfalls-138-141.md) |
| 147 | **Website (customer portal) masked date field (`/payment-frequency` "next payday", `type="search" placeholder="MM/DD/YYYY"`) corrupts its value when injected as ISO (`YYYY-MM-DD`) instead of `MM/DD/YYYY`, silently blocking Save.** Use `calculateDate()` for masked UI inputs, reserve `calculateDateISO()` for API bodies. website#153, sandbox 2026-07-01. | [03-pitfalls-138-141.md](pitfalls/03-pitfalls-138-141.md) |
| 148 | **A "Save → immediate read of the result label" page-object flow reads a stale/transient value in a chained-Save test even though the success toast already fired — chain a post-toast stability wait (spinner) before returning.** `PaymentFrequencyPage.saveFrequency()`, website#153, sandbox 2026-07-01. | [03-pitfalls-138-141.md](pitfalls/03-pitfalls-138-141.md) |
