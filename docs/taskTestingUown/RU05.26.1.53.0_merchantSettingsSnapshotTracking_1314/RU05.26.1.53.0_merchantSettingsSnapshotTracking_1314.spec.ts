/**
 * RU05.26.1.53.0_merchantSettingsSnapshotTracking_1314
 *
 * Merchant Settings Snapshot Tracking (#1314).
 ...
 *
 * SPEC (authoritative): docs/taskTestingUown/RU05.26.1.53.0_merchantSettingsSnapshotTracking_1314/
 *                       RU05.26.1.53.0_merchantSettingsSnapshotTracking_1314-spec.md
 * Scenarios:            docs/scenarios/1314-merchant-settings-snapshot.md
 ...
 *
 * Target env: qa2. NOTE: the `db` worker fixture connects to process.env.ENV
 * (NOT the per-describe `envName`), so run with `ENV=qa2` for DB assertions to
 * hit qa2. The `api`/`page` fixtures honor `test.use({ envName: 'qa2' })`.
 *
 ...
 *
 * GitLab: https://gitlab.com/uown/frontend/origination/-/work_items/1314
 */
...
// Kornerstone brand parity (ssn-test-modalities §5, INVIOLÁVEL). KS1011 is the
// object-of-test merchant — hardcoded (not pickRandomMerchantKey) per
// test-data-hierarchy "Teste de configuração de merchant". All values DB-probed
// in qa2; re-assert read-only at runtime, NEVER mutate/auto-heal.
const KS = {
...
// ── Task-scoped DB read helpers (snapshot tables are unique to #1314) ──────
...
async function assertMerchantContract(db: DatabaseHelpers): Promise<void> {
  ...
  expect(
    ...
    `terraceFinance UW config drifted from the #1314 contract — fix the merchant before running (do NOT auto-heal)`,
  ).toEqual({
    epo5: TERRACE.epo5,
    epo10: TERRACE.epo10,
...
// ── Suite ─────────────────────────────────────────────────────────────────
const TEST_NAME = 'RU05.26.1.53.0_merchantSettingsSnapshotTracking_1314';
...
test.describe(`${TEST_NAME} - qa2/terraceFinance`, { tag }, () => {
  ...
  async function setupApprovedLead(
    ...
    // qa2: refresh vendor sweeps so the approved (non-9) SSN is not spuriously denied.
    ...
    await createPreQualifiedApplication(
      ...
      td.applicant,
      ctx,
      { submitPaymentInfoViaApi: true, skipMerchantPreflight: true },
      testInfo,
    );
    expect(ctx.leadPk, 'leadPk captured').toBeTruthy();
  ...
  test.describe.serial('TC-03 immutability — merchant edits do not mutate the lead snapshot', () => {
    ...
    const cases: EditCase[] = [
      {
        id: 'TC-03a', label: 'Fraud Threshold 1→5',
        edit: (ms) => ms.fillFraudThreshold('5'),
        newLive: { fraud_threshold: 5 },
      },
      {
  ...
  test.describe.serial('TC-05 account snapshot derives from lead snapshot', () => {
    ...
    test('TC-05: account snapshot == lead snapshot after a Fraud Threshold change', async ({ page, api, db, testEnv }, testInfo) => {
      ...
      const { ctx, merchant } = await setupApprovedLead(api, db, testInfo);
      await test.step('Capture lead snapshot (fraud_threshold=1)', async () => {
        const present = await db.waitForRecord(
          'uown_los_lead_merchant_settings_snapshot', 'lead_pk = $1', [ctx.leadPk], 60_000,
        );
        expect(present).toBe(true);
      ...
      const leadSnap = (await readLeadSnapshots(db, ctx.leadPk))[0];
      expect(Number(leadSnap.fraud_threshold), 'lead snapshot captured fraud_threshold=1').toBe(1);

      await test.step('Change Fraud Threshold 1→5 via Origination Merchant Setting UI', async () => {
        await loginToPortal(page, testEnv.originationUrl, testEnv);
        const ms = new MerchantSettingPage(page);
        await ms.navigateToMerchantSettings(testEnv.originationUrl);
        await ms.loadMerchants();
        await ms.selectMerchantRowByText(TERRACE.code);
        await ms.fillFraudThreshold('5');
        await ms.saveGdsSettings();
        const live = await db.queryOne<MerchantConfigRow>(
          `SELECT fraud_threshold FROM uown_merchant WHERE pk = $1`, [TERRACE.pk],
        );
        expect(Number(live!.fraud_threshold), 'live merchant now 5 (≠ snapshot)').toBe(5);
      });
      ...
      await test.step('Account snapshot reflects APPROVAL-time value (1), not the live value (5)', async () => {
        const acctSnap = (await readAccountSnapshots(db, accountPk))[0];
        expect(Number(acctSnap.fraud_threshold), 'account snapshot fraud_threshold == 1 (approval-time)').toBe(1);
        expect(Number(acctSnap.fraud_threshold), 'account snapshot == lead snapshot').toBe(Number(leadSnap.fraud_threshold));
        expect(Number(acctSnap.fraud_threshold), 'must NOT be the new live value 5').not.toBe(5);
      });
    });
  });
...
test.describe.serial(`${TEST_NAME} - qa2/Kornerstone/KS1011`, { tag: KS_TAG }, () => {
  test.use({ envName: 'qa2' });
  ...
  async function setupApprovedKsLead(
    ...
    // qa2: refresh vendor sweeps so the approved (non-9) SSN is not spuriously
    // denied by a stale Kount/GDS token (fraud-vendors-knowledge §3).
    await api.scheduledTask.refreshKountAccessTokenSweep().catch(() => {});
    await api.scheduledTask.refreshGdsAccessTokenSweep().catch(() => {});
  ...
  // ════════════════════════════════════════════════════════════════════════
  test('TC-KS-01: KS lead snapshot on approval with 4 values (fraud_threshold=5) and COUNT=1', async ({ api, db }, testInfo) => {
    ...
    await test.step('Exactly one row with KS UW values + merchant link (merchant_pk=315)', async () => {
      ...
      expect(Number(s.fraud_threshold), 'fraud_threshold = KS value 5 (NOT 1)').toBe(KS.fraudThreshold);
