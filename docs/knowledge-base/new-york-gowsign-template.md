...
- Throwaway driver: `tests/api/__scratch_ny_signing_url_svc544.spec.ts` (`ENV=qa2`) — mints the NY 13m lead + prints the consumer signing URL. Delete after the task closes.
...
- **Automated AC suite (green — `3 passed`):** `tests/e2e/gowsign/ny-gowsign-template-svc544.spec.ts`. Run: `ENV=qa2 npx playwright test tests/e2e/gowsign/ny-gowsign-template-svc544.spec.ts --project=cross-portal`.
...
- Throwaway drivers (delete on task close): `tests/api/__scratch_ny_signing_url_svc544.spec.ts`, `tests/e2e/gowsign/__scratch_ny_signing_completion_svc544.spec.ts` (superseded by the AC spec), `src/scripts/_scratch_extract_signwell_pdf.mjs`.
