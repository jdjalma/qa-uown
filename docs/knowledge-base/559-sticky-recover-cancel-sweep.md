---
title: Sticky Recover Cancel Sweep (svc#559)
...
sources:
  ...
  - code: uown/backend/svc!1483 "Handle non-cancelable transactions" (StickyRecoverCancelService + StickyRecoverCancelSweepService)
  - code: uown/backend/sticky.io!9 "Add markRecoveryCanceled method"
...
# Sticky Recover Cancel Sweep (svc#559)

> Charter: Explore the `StickyRecoverCancelSweep` flow (svc#559) via API/DB + UI periphery to discover the cancel-recovery rules, states, and the `"cannot cancel"` → CANCELED-local branch, and which business rules it touches.
> Origin: GitLab task [uown/backend/svc#559](https://gitlab.com/uown/backend/svc/-/work_items/559) — milestone RU06.26.1.53.0 (same train as #552). MRs **merged**: svc!1483 + sticky.io!9. · Overall confidence: **high** (the fix already executed live in sandbox — see Confirmed Run).
...
- `uown_sv_activity_log` pk10961637 → `log_type=**INTERNAL**`, `created_by=**SYSTEM**`, `creation_source=**SYSTEM_GENERATED**`, `is_hidden=false`, `deleted=false`, `lead_pk=null` (account-level). `notes` = **the exact 559 note** (see below). This was the **first** "marked CANCELED locally" note in sandbox (the fix fired organically here; a second, deliberately-manufactured occurrence followed — see Confirmed Run #2). `[confirmed]`
...
> **Cross-rule (links to #552):** a later **refund** attempt on account 5084 (2026-06-21, by `manager`) failed — `uown_sv_activity_log` pk10994753: *"Sticky refund failed … error=Sticky recovery is not in RECOVERED status"*. ⇒ **a CANCELED recovery cannot be refunded** (refund gate = `RECOVERED`). `[confirmed]`

## Confirmed Run #2 — fresh end-to-end reproduction (sandbox, 2026-06-21 22:35:43Z)

Manufactured live by the svc#559 test (CT-M1-LIVE, orchestrator-monitored) — upgrades **AC-01/02/07 from "frozen record" to "fresh-reproduced"**. Account **17213** · sticky pk **35** · `sticky_transaction_id=3204b07c0db0755933066480d2d413471325548301974956e6`:
...
| Force-local-cancel | ✅ (new, #559) | ONLY when Sticky error title/message matches `"cannot cancel"` (case-insensitive) |
...
4. Branch on Sticky response:
   ...
   - **error AND title/message contains `"cannot cancel"` (case-insensitive)** → **mark `CANCELED` locally**, WARN log (sticky pk, txn id, account pk, full Sticky error), INTERNAL activity-log note, return normally (row counts as processed, **not** a sweep failure). ← **#559**
...
| RECOVERY_STARTED / PENDING → CANCELED | cancel sweep, **Sticky says "cannot cancel"** | ✅ **local sync (#559)** |
...
- **BR-07 — A CANCELED recovery cannot be refunded.** Refund (#552) requires `recovery_status=RECOVERED`; attempting to refund a CANCELED recovery fails with "Sticky recovery is not in RECOVERED status". *(evidence: account 5084 activity pk10994753)* `[confirmed]`
...
- **Confirms** the dunning/recovery model (svc#485, `src/helpers/sticky.helpers.ts`) and the #552 refund knowledge ([[sticky-payment-refund]]): cancel is the "account closed → stop chasing" counterpart of refund.
...
- **Adds** BR-07 (CANCELED ⇏ refundable) — a cross-feature constraint the #552 doc had not yet recorded.
...
1. **Sandbox DB tunnel** — `127.0.0.1:5445` (manual kubectl port-forward; ask the user to bring it up; port is reused across envs so verify it points to sandbox via `to_regclass('uown_sticky')` + a known sandbox row). Connect via **config object** (raw password) — building a URL by hand with the raw password breaks auth (special chars). `npx tsx src/scripts/_probe_559_discovery.ts sandbox 5445` dumps sweep SQL + states + candidates.
...
- 🟡 **REFUNDED/REFUND_SUBMITTED not excluded** by the sweep SQL — a non-active account with a REFUNDED recovery would be picked and a cancel attempted. Out of #559 scope, but a candidate product question (does cancel of a refunded recovery error, and does it then hit the "cannot cancel" path?). `[observation]`
