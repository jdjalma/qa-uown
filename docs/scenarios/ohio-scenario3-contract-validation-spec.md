...
3. **Customer state = OH** with the proven-good Bucyrus OH address (`1875 N Sandusky Ave, Bucyrus, 44820`) + sticky SSN `082390916` (forces EligibleTerms 16 → OH 16m SAC + GowSign).
...
- **Q1 — INSTORE routing contradiction.** The clone is INSTORE/state=CA, yet the reference lead rendered the OH (customer-state) template, contradicting the documented "INSTORE routes by merchant state" rule. CT-00 asserts the actual selected template as a gate. **For PO/dev:** is OH-by-customer-state the intended routing for this clone, or is lead 16643 an artifact? If intended, the documented routing rule needs updating (Rule #11 — implicit requirement → catalog).
