...
- **Context primary sources (do not re-derive):** Fernando's OH 16-month SAC copy spec; confirmed defect on lead 16643 (Charles Miller, $662, MONTHLY); value oracle in `tests/api/__scratch_oh_item4_4a_oracle.spec.ts`; KB `docs/knowledge-base/16m-lease-and-gowsign-signwell-routing-qa2.md`
...
From `api.invoice.sendInvoice(...).paymentDetailsList[freq]` (confirmed mapping in `tests/api/__scratch_oh_item4_4a_oracle.spec.ts`):
