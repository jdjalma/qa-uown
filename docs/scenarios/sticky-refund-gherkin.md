# Feature: Refund a Sticky-recovered payment — BDD scenarios

> Formal Given/When/Then for the Servicing agent flow. UI-first. Companion to the risk matrix `sticky-refund-scenarios.md` and the AC report `docs/reports/sticky-refund-report.md`.
...
> `uown_sv_account.pk` salvo no run live. Fonte: `sticky-refund-scenarios.md` (histórico de execução — não é fonte de pattern). Sticky só roda em sandbox ([[sticky-refund-tests-sandbox-only]]).
