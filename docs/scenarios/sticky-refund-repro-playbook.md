# Playbook de reprodução manual — riscos descobertos no Sticky refund

> Guia passo a passo para **reproduzir manualmente** cada risco que o QA descobriu ao validar a tarefa. Pensado para quem **não acompanhou a investigação** — cada risco traz contexto, o que é esperado (correto) vs o que acontece (bug), os passos e como verificar.
> Complementa: relatório executivo `docs/reports/sticky-refund-report.md` · matriz de risco `docs/scenarios/sticky-refund-scenarios.md` · conhecimento `docs/knowledge-base/sticky-payment-refund.md`.
...
Esta tarefa adiciona a capacidade de **ESTORNAR (refund)** um pagamento que foi recuperado pela Sticky — algo que o estorno normal não consegue, porque o dinheiro está do lado da Sticky.
   ...
   └─ StickyRecoverSweep cria a sessão  → uown_sticky.recovery_status = RECOVERY_STARTED
        └─ Sticky retenta (webhooks: recovery.started → attempt_failed → successful)
             └─ captura com sucesso          → RECOVERED   (agora é estornável)
                  └─ agente dá refund         → REFUND_SUBMITTED
...
1. **Confirme que o tunnel de DB aponta para o sandbox.** A porta do `kubectl port-forward` (`127.0.0.1:5445`) é **reusada entre ambientes ao longo do dia** — se apontar para o env errado, você vai ler/escrever no lugar errado.
   ...
   npx tsx src/scripts/_probe_verify.ts sandbox 5084 83562 2190619 36
   ```
...
3. **Para achar um candidato refundável** (quando o risco precisar): `npx tsx src/scripts/_probe_refund.ts sandbox` lista as sessões RECOVERED + STICKY/PAID disponíveis.
...
**Verifique:** `npx tsx src/scripts/_probe_verify.ts sandbox 5084 83562 2190619 36` → payment continua `PAID`; aparece um activity log `"Sticky refund failed … not in RECOVERED status"`; **nenhum** novo outbound `STICKY_REFUND`. (Sem cleanup — nada mudou.)
...
**Verifique:** `_probe_verify.ts sandbox 17204 83244 2190591 34` → sessão `REFUNDED`, payment `PAID`.
...
1. `npx tsx src/scripts/_probe_terminal.ts sandbox set 6169 SETTLED_IN_FULL`  *(ou CHARGED_OFF | CANCELLED | SOLD)*
...
4. **Cleanup:** `npx tsx src/scripts/_probe_terminal.ts sandbox restore 6169`.

**Destrutivo (consome 6169 — só se precisar do dano ao vivo):** em vez do Cancel, dê **Submit** → "Successfully refunded payment." Depois verifique com `_probe_terminal.ts sandbox show 6169`:
...
Cleanup: `_probe_terminal.ts sandbox restore 6169`.
...
```bash
npx tsx src/scripts/_probe_repro_h3.ts sandbox     # cria conta nova, prova gate + ausência de guardião, e limpa sozinho
npx tsx src/scripts/_probe_sweepsql.ts sandbox      # (opcional) dump dos 2 sweeps: confirme que o cancel sweep não filtra rating
```
...
```bash
npx tsx src/scripts/_probe_newscenarios.ts sandbox   # dailyTaxCloudRefundsSync → is_active=false (o de pagamentos está ativo)
npx tsx src/scripts/_probe_alloc_pp.ts  sandbox      # payments REVERSED têm 0 linhas em uown_sv_allocation (tax apagado)
```
...
```bash
npx tsx src/scripts/_probe_rating.ts sandbox     # contas 17177 (F) e 17176 (M) têm session Sticky
```
...
1. Numa sessão `RECOVERY_STARTED` (ex.: 17279 sticky pk=50 / cct 85949), force o estado:
   ```bash
   npx tsx src/scripts/_probe_force.ts 50 85949      # insere retry APPROVED + SvPayment STICKY/PAID + marca RECOVERED; imprime o novo paymentPk
   ```
...
5. **Cleanup:** `npx tsx src/scripts/_probe_unforce.ts`   *(remove o SvPayment + retry fabricados e reverte a sessão para RECOVERY_STARTED)*.
...
| **C-2/C-3** sem state guard | 🔴 | UI + flip DB | candidato 6169 | `_probe_terminal.ts` + UI |
| **H-3** guardião de rating | 🔴 | script SQL | — | `_probe_repro_h3.ts` |
| **N-3/C-7** imposto/de-allocation | 🔴 | observação DB | — | `_probe_newscenarios.ts` · `_alloc_pp.ts` |
| **H-2** fraud/sold não excluídos | 🟡 | observação DB | — | `_probe_rating.ts` |
| **L-1** refund só em RECOVERED real | 🔴 | DB force + UI | sessão RECOVERY_STARTED | `_probe_force.ts` + UI + `_unforce.ts` |
...
Script que faz isso para uma lista de ccts com gateway txn real: `npx tsx src/scripts/_probe_morphauth.ts <accountPk> <cctPk,cctPk,...>` (morfa + limpa rating + dispara o sweep + reporta as sessões criadas).
