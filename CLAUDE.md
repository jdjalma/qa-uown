...
19. **BDD Oracle — validate every named operation against its acceptance contract.** Full protocol in [`docs/scenarios/_index.md`](docs/scenarios/_index.md). **Trigger: do NOT rely on keyword matching alone** — after any action that produces a state change in a portal (navigation to a new route, form submission, button click that triggers a backend call), check `_index.md` regardless of how the request was phrased. If the outcome matches a listed operation, the oracle applies. Summary: **(a) Operation listed in `_index.md`:** read the BDD file → run staleness check (command embedded in each `### Oracle` section) → if stale, prepend `[BDD MAY BE STALE]` → validate every checkpoint → report PASS/FAIL before declaring the action complete. **(b) Operation NOT listed:** proceed, then append `[UNVALIDATED — no BDD oracle registered]`; in a QA pipeline context (writing/modifying tests), STOP and create the BDD first via `test-scenarios` skill. **(c) Oracle checkpoint FAILS:** inspect real DOM first → check if `covers` files changed intentionally → if yes, BDD is stale (update it); if no, report as `[BUG]`. Never confirm a bug without ruling out BDD staleness. **(d) Orchestrator dispatching an agent** that will perform a listed operation: include `> BDD Oracle applies (rule #19) — read docs/scenarios/_index.md before marking the operation complete.` in the task prompt. Applies to every agent and the orchestrator, including ad-hoc requests. Origin: 2026-06-26.
...
| BDD scenarios + Oracle index (Gherkin + post-action validation, rule #19) | [`docs/scenarios/_index.md`](docs/scenarios/_index.md) · [`docs/scenarios/`](docs/scenarios/) |
