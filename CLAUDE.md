...
19. **BDD Oracle — validate every named operation against its acceptance contract.** Full protocol in [`.claude/oracles/_index.md`](.claude/oracles/_index.md). **Trigger: do NOT rely on keyword matching alone** — after ANY operation performed in a portal, check `_index.md` regardless of how the request was phrased. **There is NO read-only exemption (decision 2026-06-30):** viewing a report/feed/list, filtering or searching a table, opening a lookup, exporting a CSV, and plain navigation to a new route ALL count as operations that require a registered oracle — exactly the same as form submissions, button clicks that trigger a backend call, **or running `npx playwright test` on any spec that exercises a registered operation**. "It's just a read" is NEVER a reason to skip the oracle. **Running an existing spec IS performing the operations it exercises — it is not exempt.** **Probes, ad-hoc/exploratory Playwright scripts, and one-off `*.spec.ts` files are operations too (decision 2026-06-30): the oracle pre-check runs BEFORE you write/run them, not after.** A probe IS the execution of the operation (it authenticates, navigates a route, and reads/mutates the portal) — consulting `_index.md` only after the probe already ran, or because a hook forced it, is a protocol violation: the operation has already happened. If the probe exercises an unlisted operation, STOP and create the oracle first (rule #19(b)), then let the probe validate its checkpoints. There is no "it's just a quick probe to look" exemption. Classifying "run this test" as purely a technical operation to skip BDD is a protocol violation. If the outcome matches a listed operation, the oracle applies. Summary: **(a) Operation listed in `_index.md`:** read the BDD file → run staleness check (command embedded in each `### Oracle` section) → if stale, prepend `[BDD MAY BE STALE]` → validate every checkpoint → report PASS/FAIL before declaring the action complete. **(b) Operation NOT listed (ANY context — ad-hoc request OR QA pipeline, no distinction):** STOP before performing. Author the BDD first via the `test-scenarios` skill (ground checkpoints in the canonical business rules; run a `discovery` pass per rule #18 if the expected behavior is unknown), register it in `_index.md` (new row + `covers`/`last-reviewed` frontmatter), THEN perform the operation and validate every checkpoint → report `Oracle: CT-XX — PASS/FAIL`. **Nothing reaches a validated state without an oracle** — the former "proceed and append `[UNVALIDATED — no BDD oracle registered]`" escape hatch is RETIRED. An unlisted operation is never a license to proceed unvalidated; it is the trigger to create the missing oracle first — this holds whether the operation mutates state or is a pure read (feed / report / list / search / lookup / CSV export). **(c) Oracle checkpoint FAILS:** inspect real DOM first → check if `covers` files changed intentionally → if yes, BDD is stale (update it); if no, report as `[BUG]`. Never confirm a bug without ruling out BDD staleness. **(d) Orchestrator dispatching an agent** that will perform a listed operation: include `> BDD Oracle applies (rule #19) — read .claude/oracles/_index.md before marking the operation complete.` in the task prompt. **(e) "Create a lease" / "perform X in portal" vs "run the test for X":** these are different requests with the same oracle obligation — do not conflate them. "Create a lease via new application" = manual portal operation via MCP Playwright; "run new-application.spec.ts" = automated test execution. Both require oracle pre-check. Applies to every agent and the orchestrator, including ad-hoc requests. Origin: 2026-06-26; extended 2026-06-26 (Bash/playwright test gap); (b) unified to STOP-and-create for ad-hoc + `[UNVALIDATED]` escape retired 2026-06-27; read-only exemption removed 2026-06-30 — every portal operation, including pure reads (report/feed/list/search/lookup/CSV), requires a registered oracle (origin: `/alerts` feed read performed without an oracle under the old state-changing-only scope).
