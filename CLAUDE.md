...
| `subagent-validate-results` | green | Validates test results + generates task report artifact (`.md`) | No (writes report) |
...
8. **Task report `.md` MUST be updated after every test execution** — never leave PENDING values after a successful run
